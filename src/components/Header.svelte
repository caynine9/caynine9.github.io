<script lang="ts">
  ...
  function getLanguageUrl(lang: string) {
    if (lang === 'id') {
      return currentPath.startsWith('/id') ? currentPath : '/id' + (currentPath === '/' ? '' : currentPath);
    } else {
      return currentPath.startsWith('/id') ? currentPath.replace('/id', '') || '/' : currentPath;
    }
  }
</script>

<header class="sticky top-0 z-50 glass transition-colors duration-500">
  <nav class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative h-14 flex items-center justify-between">
    ...
    <div class="flex items-center space-x-2 ml-auto">
      <!-- Language Switcher -->
      <div class="flex items-center bg-black/10 dark:bg-white/5 rounded-full p-1 mr-2">
        <a 
          href={getLanguageUrl('id')}
          class="px-2 py-1 rounded-full text-[10px] font-bold transition-all {currentLang === 'id' ? 'bg-white text-black dark:bg-gray-800 dark:text-white shadow-sm opacity-100' : 'opacity-40 hover:opacity-70'}"
        >
          ID
        </a>
        <a 
          href={getLanguageUrl('en')}
          class="px-2 py-1 rounded-full text-[10px] font-bold transition-all {currentLang === 'en' ? 'bg-white text-black dark:bg-gray-800 dark:text-white shadow-sm opacity-100' : 'opacity-40 hover:opacity-70'}"
        >
          EN
        </a>
      </div>
