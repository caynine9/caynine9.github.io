...
<div class="fixed top-0 left-0 right-0 z-50 flex justify-center p-3 sm:p-6 pointer-events-none transition-all duration-300">
  <header class="pointer-events-auto w-full max-w-5xl {isMenuOpen ? 'glass rounded-2xl p-5 shadow-2xl border border-[var(--border)] bg-[var(--bg)]/95 backdrop-blur-xl' : (scrolled ? 'glass rounded-2xl sm:rounded-full px-5 py-3 shadow-lg border border-[var(--border)]' : 'glass sm:bg-transparent rounded-2xl sm:rounded-none px-5 sm:px-2 py-3 sm:py-4')} transition-all duration-300 ease-out flex flex-col">
    ...
    <!-- Mobile Menu Container (Inside Header Card) -->
    {#if isMenuOpen}
      <div transition:slide={{ duration: 250 }} class="sm:hidden pt-4 mt-3 border-t border-[var(--border)] space-y-1">
        <a
        ...
        <a
          ...
          class="block px-4 py-3 rounded-xl text-sm font-semibold transition-colors {isActive(currentLang === 'id' ? '/id/blog' : '/blog') ? 'bg-[var(--accent-glow)] text-[var(--accent)]' : 'text-[var(--text)] hover:bg-black/5 dark:hover:bg-white/5'}"
          on:click={() => (isMenuOpen = false)}
        >
          Logcat
        </a>
